...
def backtracking_min(f, x_0, method, alpha_max=1, rho=0.5, c=0.5, iter=0,
    ...
    f_obj = np.zeros((max_iter, 1))
    alpha = alpha_max * np.ones((max_iter, 1))
    alpha[0] = 0

    x, f_obj[iter] = x_0.copy(), f[0](x_0)
    while np.linalg.norm(f[1](x)) > tol and iter < max_iter:
        iter += 1
...
def step_dir(f, x_k, method):
    ...
    if method == 'newton' or method == 'n':
        p = - np.dot(np.linalg.inv(f[2](x_k)), f[1](x_k))
...
def step_length(f, x_k, p_k, alpha_max=1, rho=0.5, c=0.5):
    ...
    while (f[0](x_k + alpha * p_k) > 
           f_x_k + c * alpha * np.dot(f[1](x_k).T, p_k)):
        alpha = rho * alpha
    return alpha, f_x_k
...
def rb_function(x):
    return np.array([(100*(x[1, 0] - x[0, 0]**2)**2 + (1 - x[0, 0])**2)])

def rb_gradient(x):
    return np.array([[2*x[0, 0] - 400*x[0, 0] * (-x[0, 0]**2 + x[1, 0]) - 2],
                     [200*(x[1, 0] - x[0, 0]**2)]])

def rb_hessian(x):
    return np.array([[(2 + 1200*x[0, 0]**2 - 400*x[1, 0]), (-400*x[0, 0])],
                     [(-400*x[0]), 200]])
